...
public class RMRssShuffleReaderTest {
  ...
  public void testReadShuffleWithoutCombine() throws Exception {
    ...
    RMRecordsReader recordsReader = new RMRecordsReader(APP_ID, SHUFFLE_ID, Sets.newHashSet(PARTITION_ID),
        ImmutableMap.of(PARTITION_ID, serverInfos), rssConf, keyClass, valueClass, comparator, combiner,
        false, inc -> readMetrics.incRecordsRead(inc));
    ...
    RMRssShuffleReader shuffleReader =
        new RMRssShuffleReader(PARTITION_ID, PARTITION_ID + 1, context, handle, writeClient,
            ImmutableMap.of(PARTITION_ID, serverInfos),
            ImmutableMap.of(PARTITION_ID, Roaring64NavigableMap.bitmapOf(blockIds)),
            Roaring64NavigableMap.bitmapOf(taskAttemptId), readMetrics, rssConf);
  ...
  public void testReadMulitPartitionShuffleWithoutCombine() throws Exception {
    ...
    // 3 construct remote merge records reader
    ShuffleReadMetrics readMetrics = new ShuffleReadMetrics();
    RMRecordsReader recordsReader = new RMRecordsReader(APP_ID, SHUFFLE_ID,
        ...
        rssConf, keyClass, valueClass, comparator, combiner, false,
        inc -> {synchronized (this) {readMetrics.incRecordsRead(inc);}});
    RMRecordsReader recordsReaderSpy = spy(recordsReader);
    ...
    ShuffleWriteClient writeClient = mock(ShuffleWriteClient.class);
    RMRssShuffleReader shuffleReader =
        new RMRssShuffleReader(PARTITION_ID, PARTITION_ID + 3, context, handle, writeClient,
            ImmutableMap.of(PARTITION_ID, serverInfos, PARTITION_ID + 1, serverInfos, PARTITION_ID + 2, serverInfos),
            ImmutableMap.of(PARTITION_ID, Roaring64NavigableMap.bitmapOf(blockIds[0], blockIds[1]),
                PARTITION_ID + 1, Roaring64NavigableMap.bitmapOf(blockIds[2], blockIds[3]),
                PARTITION_ID + 2, Roaring64NavigableMap.bitmapOf(blockIds[4], blockIds[5])),
            Roaring64NavigableMap.bitmapOf(taskAttemptId), readMetrics, rssConf);
    RMRssShuffleReader shuffleReaderSpy = spy(shuffleReader);
    doReturn(recordsReaderSpy).when(shuffleReaderSpy).createRMRecordsReader(anySet(), anyMap());
