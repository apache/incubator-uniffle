...
public class RMRssShuffleReader<K, C> implements ShuffleReader<K, C> {
  ...
  public RMRssShuffleReader(
    ...
    try {
      ...
      this.valueClass = isMapCombine ?
          ClassUtils.getClass(rssShuffleHandle.getDependency().combinerClassName().getOrElse(() -> {
            throw new RssException("Can not find combine class even though map combine is enabled!");
          }))
          : ClassUtils.getClass(rssShuffleHandle.getDependency().valueClassName());
      String comparatorClassName =
          rssShuffleHandle.getDependency().keyOrdering().map(o -> o.getClass().getName()).getOrElse(() -> null);
      if (comparatorClassName != null) {
        Class comparatorClass = ClassUtils.getClass(comparatorClassName);
  ...
  private void reportUniqueBlocks(Set<Integer> partitionIds) {
    for (int partitionId : partitionIds) {
      ...
      shuffleWriteClient.reportUniqueBlocks(new HashSet<>(partitionToServerInfos.get(partitionId)),
          appId, shuffleId, partitionId, uniqueBlockIdBitMap);
    }
  }
  ...
  class MultiPartitionIterator<K, C> extends AbstractIterator<Product2<K, C>> {
    ...
    MultiPartitionIterator() {
      ...
      this.dataIterator = CompletionIterator$.MODULE$.apply(iter, () -> {
  ...
  @VisibleForTesting
  public RMRecordsReader createRMRecordsReader(Set<Integer> partitionIds,
                                               Map<Integer, List<ShuffleServerInfo>> serverInfoMap) {
    return new RMRecordsReader(appId, shuffleId, partitionIds, serverInfoMap, rssConf, keyClass, valueClass, comparator,
        combiner, isMapCombine, inc -> {
