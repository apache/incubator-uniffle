...
/**
 * Remote Merge merges the original blocks into a new block set starting with id 1.
 * For now, all blocks under the partition are written into a file. Remote Merge needs to read
 * the contents of each block separately. If we use a file handle to manage each block, a large
 * number of open files will be wasted. Therefore, BlockFlushFileReader was introduced.
 *
 * BlockFlushFileReader uses one file handle to manage all blocks under this partition.
 * The FlushFileReader thread is used to read this file corresponding to the partition.
 * FlushFileReader reads the partial buffer of each block in sequence each time.
 * In this way, FlushFileReader always reads data in the order of increasing offset,
 * which reduces random reads compared to opening a file per block.
 * BlockInputStream reads the buffer corresponding to the block. We use RingBuffer to
 * balance the buffer generated by FlushFileReader and the buffer consumed by BlockInputStream.
 */
