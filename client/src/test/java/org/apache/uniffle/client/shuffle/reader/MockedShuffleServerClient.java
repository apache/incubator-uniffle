...
public class MockedShuffleServerClient implements ShuffleServerClient {
  ...
  public RssGetSortedShuffleDataResponse getSortedShuffleData(RssGetSortedShuffleDataRequest request) {
    ...
    if (index < shuffleData.get(partitionId).size()) {
      ...
      response = new RssGetSortedShuffleDataResponse(StatusCode.SUCCESS, shuffleData.get(partitionId).get(index),
          10000, MergeState.DONE.code());
    } else {
      response = new RssGetSortedShuffleDataResponse(StatusCode.SUCCESS, null, -1, MergeState.DONE.code());
    }
