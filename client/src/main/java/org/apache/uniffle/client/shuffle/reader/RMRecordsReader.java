...
public class RMRecordsReader<K, V, C> {
  ...
  public RMRecordsReader(
    ...
    this.maxRecordsNumPerBuffer = rssConf.get(RSS_CLIENT_REMOTE_MERGE_READER_MAX_RECORDS_PER_BUFFER);
  ...
  class RecordsFetcher extends Thread {
    ...
    RecordsFetcher(int partitionId) {
      ...
      this.nextQueue = combiner == null ? mergeBuffers.get(partitionId) : combineBuffers.get(partitionId);
      ...
      while (!stop) {
        try {
          ...
          if (response.getStatusCode() != StatusCode.SUCCESS ||
            ...
            nextShuffleServerInfo();
            break;
          } if (response.getMergeState() == MergeState.MERGING.code() && response.getNextBlockId() == -1) {
            // All merged data has been read, but there may be data that has not yet been merged. So wait done!
            LOG.info("RMRecordsFetcher will sleep {} ms", sleepTime);
            Thread.sleep(this.sleepTime);
            this.sleepTime = Math.min(this.sleepTime * 2, maxFetchSleepTime);
          } else if (response.getMergeState() == MergeState.DONE.code() && response.getNextBlockId() == -1) {
            ...
            // Fetch blocks and parsing blocks are a synchronous process. If the two processes are split into two
  ...
  class RecordsCombiner extends Thread {
    ...
    public void run() {
      while (!stop) {
        try {
          ...
          RecordBuffer<K, V> current = combineBuffers.get(partitionId).take();
          ...
          if (current == null) {
            ...
            // 3 combine the current, then cache it. By this way, we can handle the specical case that next record
            // buffer has same key in current.
            RecordBlob<K, V, C> recordBlob = new RecordBlob(partitionId);
            ...
            for (Record<K, C> record : recordBlob.getResult()) {
              if (cached.size() >= maxRecordsNumPerBuffer &&
                  !isSameKey(record.getKey(), cached.getLastKey())) {
                sendCachedBuffer(cached);
                cached = new RecordBuffer<>(partitionId);
              }
              cached.addRecord(record);
            }
          }
        } catch (InterruptedException e) {
    ...
    private void sendCachedBuffer(RecordBuffer<K, C> cachedBuffer) throws InterruptedException {
      ...
      RecordBlob<K, C, C> recordBlob = new RecordBlob(partitionId);
      recordBlob.addRecords(cachedBuffer);
      recordBlob.combine(combiner, true);
      RecordBuffer<K, C> recordBuffer = new RecordBuffer<>(partitionId);
  ...
  class RecordsMerger extends Thread {
    ...
    public void run() {
      try {
        ...
        Merger.MergeQueue mergeQueue = new Merger.MergeQueue(rssConf, segments, keyClass, valueClass, comparator);
        mergeQueue.init();
  ...
  public ShuffleServerClient createShuffleServerClient(ShuffleServerInfo shuffleServerInfo) {
    return ShuffleServerClientFactory.getInstance().getShuffleServerClient(RSS_CLIENT_TYPE_DEFAULT_VALUE,
