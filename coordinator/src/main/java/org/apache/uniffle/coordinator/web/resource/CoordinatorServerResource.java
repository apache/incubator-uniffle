...
public class CoordinatorServerResource extends BaseResource {
  ...
  @GET
  @Path("/info")
  public Response<List> getCoordinatorInfo() {
    return execute(() -> {
      ...
      for (Map.Entry<String, Object> stringObjectEntry : allEntry) {
        CoordinatorConfVO result = new CoordinatorConfVO(stringObjectEntry.getKey(),
  ...
  public Response<Map> getCoodConf() {
    return execute(() -> {
      ...
      coordinatorServerInfo.put("coordinatorId", coordinatorConf.getString(CoordinatorUtils.COORDINATOR_ID,"none"));
      ...
      coordinatorServerInfo.put("serverWebPort", String.valueOf(coordinatorConf.getInteger("rss.jetty.http.port", 0)));
      return coordinatorServerInfo;
    });
  }

  private CoordinatorServer getCoordinatorServer() {
    return (CoordinatorServer) servletContext.getAttribute(
        CoordinatorServer.class.getCanonicalName());
  }
}
