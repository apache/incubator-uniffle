...
public class RMRssShuffleTest {
  ...
  public void testReadShuffleData() throws Exception {
    ...
    TezTaskAttemptID tezTaskAttemptID = TezTaskAttemptID.getInstance(TezTaskID.getInstance(TezVertexID.getInstance(
        TezDAGID.getInstance(APPLICATION_ATTEMPT_ID.getApplicationId(), 0), 0), 0), 0);
    when(inputContext.getUniqueIdentifier()).thenReturn(String.format("%s_%05d", tezTaskAttemptID.toString(), 0));
    ...
    when(inputContext.getServiceProviderMetaData(anyString())).thenReturn(
        ByteBuffer.wrap(dataOutputBuffer.getData(), 0, dataOutputBuffer.getLength()));
    ...
    RMRecordsReader recordsReader =
        new RMRecordsReader(APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID, Sets.newHashSet(PARTITION_ID),
            ImmutableMap.of(PARTITION_ID, serverInfos), rssConf, keyClass, valueClass, comparator, null, false, null);
    RMRecordsReader recordsReaderSpy = spy(recordsReader);
    ByteBuffer[][] buffers = new ByteBuffer[][] {
        {ByteBuffer.wrap(genSortedRecordBytes(rssConf, keyClass, valueClass, 0, 1, RECORDS_NUM, duplicated))}};
    ShuffleServerClient serverClient = new MockedShuffleServerClient(new int[] {PARTITION_ID}, buffers, blockIds);
    ...
    try (MockedStatic<UmbilicalUtils> umbilicalUtils = Mockito.mockStatic(UmbilicalUtils.class);
      ...
      writeClient.reportShuffleResult(
          ImmutableMap.of(serverInfos.get(0), ImmutableMap.of(PARTITION_ID, Sets.newHashSet(blockIds[0]))),
          APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID, taskAttemptId, 0);
      tezUtils.when(() -> RssTezUtils.createShuffleClient(any())).thenReturn(writeClient);
      tezUtils.when(() -> RssTezUtils.fetchAllRssTaskIds(anySet(), anyInt(), anyInt())).thenReturn(
      ...
      for (int i = 0; i < 5; i++) {
        events.add(createDataMovementEvent(PARTITION_ID, "attempt_" + i));
      }
  ...
  public void testReadMultiPartitionShuffleData() throws Exception {
    ...
    // 3 mock recordsReader
    RMRecordsReader recordsReader = new RMRecordsReader(APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID,
        Sets.newHashSet(PARTITION_ID, PARTITION_ID + 1, PARTITION_ID + 2),
        ImmutableMap.of(PARTITION_ID, serverInfos, PARTITION_ID + 1, serverInfos, PARTITION_ID + 2, serverInfos),
        rssConf, keyClass, valueClass, comparator, null, false, null);
    RMRecordsReader recordsReaderSpy = spy(recordsReader);
    ByteBuffer[][] buffers = new ByteBuffer[3][2];
    for (int i= 0; i < 3; i ++) {
      buffers[i][0] = ByteBuffer.wrap(genSortedRecordBytes(rssConf, keyClass, valueClass, i, 3, RECORDS_NUM,
          duplicated));
      buffers[i][1] = ByteBuffer.wrap(genSortedRecordBytes(rssConf, keyClass, valueClass, i + RECORDS_NUM * 3, 3,
          RECORDS_NUM, duplicated));
    }
    ShuffleServerClient serverClient =
        new MockedShuffleServerClient(new int[] {PARTITION_ID, PARTITION_ID + 1, PARTITION_ID + 2}, buffers, blockIds);
    doReturn(serverClient).when(recordsReaderSpy).createShuffleServerClient(any());
    ...
    try (MockedStatic<UmbilicalUtils> umbilicalUtils = Mockito.mockStatic(UmbilicalUtils.class);
         MockedStatic<RssTezUtils> tezUtils = Mockito.mockStatic(RssTezUtils.class)) {
      umbilicalUtils.when(() -> UmbilicalUtils.requestShuffleServer(any(), any(), any(), anyInt())).thenReturn(
          ImmutableMap.of(PARTITION_ID, serverInfos, PARTITION_ID + 1, serverInfos, PARTITION_ID + 2, serverInfos));
      ShuffleWriteClient writeClient = new MockedShuffleWriteClient();
      writeClient.reportShuffleResult(ImmutableMap.of(serverInfos.get(0),
              ImmutableMap.of(PARTITION_ID, Sets.newHashSet(blockIds[0], blockIds[1]),
                  PARTITION_ID + 1, Sets.newHashSet(blockIds[2], blockIds[3]),
                  PARTITION_ID + 2, Sets.newHashSet(blockIds[4], blockIds[5]))),
          APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID, taskAttemptId, 0);
      tezUtils.when(() -> RssTezUtils.createShuffleClient(any())).thenReturn(writeClient);
      tezUtils.when(() -> RssTezUtils.fetchAllRssTaskIds(anySet(), anyInt(), anyInt())).thenReturn(
          Roaring64NavigableMap.bitmapOf(taskAttemptId));
  ...
  public static DataMovementEvent createDataMovementEvent(int partition, String path) throws Exception {
    ...
    DataOutputBuffer dataOutputBuffer = new DataOutputBuffer();
    dataOutputBuffer.writeInt(-1);
    when(context.getServiceProviderMetaData(anyString())).thenReturn(
        ByteBuffer.wrap(dataOutputBuffer.getData(), 0, dataOutputBuffer.getLength()));
    Method method = ShuffleUtils.class.getDeclaredMethod("generateDMEPayload",
        new Class[] {boolean.class, int.class, TezSpillRecord.class, OutputContext.class, int.class, boolean.class,
            boolean.class, String.class, String.class, Deflater.class});
    method.setAccessible(true);
    ByteBuffer byteBuffer = (ByteBuffer) method.invoke(null, false, 0, null, context, -1, true, true, path,
        "mapreduce_shuffle", TezCommonUtils.newBestCompressionDeflater());
