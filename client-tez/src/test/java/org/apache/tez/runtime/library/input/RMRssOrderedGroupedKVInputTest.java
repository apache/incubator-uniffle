...
public class RMRssOrderedGroupedKVInputTest {
  ...
  public void testRMRssOrderedGroupedKVInput() throws Exception {
    ...
    doAnswer(
        ...
        }).when(inputContext).requestInitialMemory(anyLong(), any(MemoryUpdateCallback.class));
    ...
    try (MockedStatic<UmbilicalUtils> umbilicalUtils = Mockito.mockStatic(UmbilicalUtils.class);
      ...
      writeClient.reportShuffleResult(serverToPartitionToBlockIds, APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID,
          taskAttemptId, 0);
      ...
      tezUtils.when(() -> RssTezUtils.uniqueIdentifierToAttemptId(anyString())).thenReturn(tezTaskAttemptID.toString());
  ...
  public void testRMRssOrderedGroupedKVInputMulitPartition() throws Exception {
    ...
    // 3 mock recordsReader
    RMRecordsReader recordsReader =
        new RMRecordsReader(APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID,
    ...
    try (MockedStatic<UmbilicalUtils> umbilicalUtils = Mockito.mockStatic(UmbilicalUtils.class);
      ...
      umbilicalUtils.when(() -> UmbilicalUtils.requestShuffleServer(any(), any(), any(), anyInt())).thenReturn(
          ImmutableMap.of(PARTITION_ID, serverInfos, PARTITION_ID + 1, serverInfos, PARTITION_ID + 2, serverInfos));
      ShuffleWriteClient writeClient = new MockedShuffleWriteClient();
      Map<ShuffleServerInfo, Map<Integer, Set<Long>>> serverToPartitionToBlockIds = ImmutableMap.of(serverInfos.get(0),
          ImmutableMap.of(PARTITION_ID, Sets.newHashSet(blockIds[0], blockIds[1]), PARTITION_ID + 1,
              Sets.newHashSet(blockIds[2], blockIds[3]), PARTITION_ID + 2, Sets.newHashSet(blockIds[4], blockIds[5])));
      writeClient.reportShuffleResult(serverToPartitionToBlockIds, APPLICATION_ATTEMPT_ID.toString(), SHUFFLE_ID,
          taskAttemptId, 0);
      tezUtils.when(() -> RssTezUtils.createShuffleClient(any())).thenReturn(writeClient);
      tezUtils.when(() -> RssTezUtils.fetchAllRssTaskIds(anySet(), anyInt(), anyInt())).thenReturn(
      ...
      List<Event> events = new ArrayList<>();
      for (int i = 0; i < 2; i++) {
        events.add(createDataMovementEvent(i / 2, "attempt_" + i));
      }
      inputSpy.handleEvents(events);
      inputSpy.start();
      events.clear();
  ...
  private static DataMovementEvent createDataMovementEvent(int partition, String path) {
    ShuffleUserPayloads.DataMovementEventPayloadProto proto =
        ShuffleUserPayloads.DataMovementEventPayloadProto.newBuilder().setPathComponent(path).build();
