...
public class TezRemoteShuffleManager implements ServicePluginLifecycle {
  ...
  private ShuffleAssignmentsInfo getShuffleWorks(int partitionNum, int shuffleId) {
    ...
    String storageType = RssTezUtils.getString(conf, conf, RssTezConfig.RSS_STORAGE_TYPE);
    boolean testMode = RssTezUtils.getBoolean(conf, conf, RssTezConfig.RSS_TEST_MODE_ENABLE, false);
