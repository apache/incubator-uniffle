...
public class RssOrderedGroupedKVInput extends AbstractLogicalInput {
  ...
  public synchronized List<Event> initialize() throws IOException {
    ...
    getContext().requestInitialMemory(initialMemoryRequest, memoryUpdateCallbackHandler); // TODO
  ...
  RssShuffle createRssShuffle() throws IOException {
    long initialMemoryAvailable = conf.getLong(RssTezConfig.RSS_REDUCE_INITIAL_MEMORY, 200 * 1024 * 1024L);
    LOG.info("RssOrderedGroupedKVInput, initialMemoryAvailable from config:{}, final initialMemoryAvailable:{}",
        conf.get(RssTezConfig.RSS_REDUCE_INITIAL_MEMORY), initialMemoryAvailable);
  ...
  // todo 可修改为按照partition的进度，不是很好改，先留着
  @Override
  ...
  public void handleEvents(List<Event> inputEvents) throws IOException {
    LOG.info("RssOrderedGroupedKVInput, handle event:{}", inputEvents);
