...
//  private InputHost inputHost ;
  ...
  public RssTezFetcherTask(FetcherCallback fetcherCallback, InputContext inputContext, Configuration conf, FetchedInputAllocator inputManager, int partition,
    ...
    this.storageType = RssTezUtils.getString(conf, conf, RssTezConfig.RSS_STORAGE_TYPE); //RssTezUtils.getString(rssJobConf, mrJobConf, RssTezConfig.RSS_STORAGE_TYPE);
...
//    final RssEventFetcher eventFetcher = new RssEventFetcher(inputs, numPhysicalInputs);
    ...
    LOG.info("------------------inputs:{}, numinput:{}, apptatmeid:{},--taskIdBitmap:{}", inputs, numPhysicalInputs, appAttemptId, taskIdBitmap);
    // todo 多个顶点，不需要考虑，
      ...
      if (!taskIdBitmap.isEmpty()) {
        ...
        CreateShuffleReadClientRequest request = new CreateShuffleReadClientRequest(
            appId, RssTezUtils.computeShuffleId(dagIdentifier, sourceVertexName, taskVertexName), partition, storageType, basePath, indexReadLimit, readBufferSize,
