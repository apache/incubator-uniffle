...
public class RssShuffleManager extends ShuffleManager{
  ...
  private final AtomicInteger numAddedPendingHosts = new AtomicInteger(0);
  ...
  private Map<Integer, Set<ShuffleServerInfo>> rssPartitionWorkers = new HashMap<>();
  ...
  public RssShuffleManager(InputContext inputContext, Configuration conf, int numInputs,
    ...
    this.numInputs = numInputs; // 上游有多少个map task数量
  ...
  private boolean isAllInputFetched() {
    LOG.info("Check isAllInputFetched, numNoDataInput:{}, numWithDataInput:{},numInputs:{},  successRssPartitionSet:{}, allRssPartition:{}.",
        numNoDataInput, numWithDataInput, numInputs, successRssPartitionSet, allRssPartition);
    return (numNoDataInput.get() + numWithDataInput.get() >= numInputs) && (successRssPartitionSet.size() >= allRssPartition.size());
  }

  private boolean isAllInputAdded() {
    LOG.info("Check isAllInputAdded, numNoDataInput:{}, numWithDataInput:{},numInputs:{},  successRssPartitionSet:{}, allRssPartition:{}.",
        numNoDataInput, numWithDataInput, numInputs, successRssPartitionSet, allRssPartition);
  ...
  private class RssRunShuffleCallable extends CallableWithNdc<Void> {
    ...
    public RssRunShuffleCallable(Configuration conf) {
      this.conf = conf;
    }

    @Override
    protected Void callInternal() throws Exception {
      // 1. 将所有的pendingHosts进行格式换换
      while (!isShutdown.get() && !isAllInputFetched()) {
        ...
        try { //  两边保持一致
          ...
          while (!isAllInputAdded() || ((rssRunningFetchers.size() >= numFetchers || pendingPartition.isEmpty()) &&
              (successRssPartitionSet.size() < allRssPartition.size()))) {
            LOG.info("isAllInputAdded:{}, rssRunningFetchers:{}, numFetchers:{}, pendingPartition:{}, successRssPartitionSet:{}, allRssPartition:{} ",
                isAllInputAdded(), rssRunningFetchers, numFetchers, pendingPartition, successRssPartitionSet, allRssPartition);
        ...
        if (successRssPartitionSet.size() < allRssPartition.size() && !isShutdown.get()) {
          lock.lock();
          try {
            ...
            while (pendingPartition.peek() != null && !isShutdown.get()) {
              ...
              try {
                partition = pendingPartition.take();  // todo： 将partition取出，失败则将partition添加进来的逻辑
              ...
              // todo：构造Fetcher任务，并提交

              if (!isShutdown.get() && (!successRssPartitionSet.contains(partition) && !runningRssPartitionMap.contains(partition))) {
//                Fetcher fetcher = constructFetcherForHost(inputHost, conf);  // 构造fetch任务，以Partition/worker为粒度，直接new对象
//                RssTezFetcherTask fetcher = constructRssFetcherForPartition(inputHost, conf);  // 构造fetch任务，以Partition/worker为粒度，直接new对象
                runningRssPartitionMap.add(partition);
                LOG.info("generate RssTezFetcherTask, partition:{}, woker:{}, all woker:{}",
                    partition,rssPartitionWorkers.get(partition),rssPartitionWorkers);

                RssTezFetcherTask fetcher = new RssTezFetcherTask(RssShuffleManager.this, inputContext, conf,
                    inputManager, partition, partitionToInput.get(partition), rssPartitionWorkers.get(partition),
                    rssAllBlockIdBitmapMap, rssSuccessBlockIdBitmapMap, numInputs, codec);
                ...
                ListenableFuture<FetchResult> future = fetcherExecutor.submit(fetcher);    // 提交fetcher任务
      ...
      LOG.info("RssShuffleManager, numAddedPendingHosts:{}, numInputs:{}", numAddedPendingHosts.get(), numInputs);
...
//  // todo， 构造fetcher 任务，待修改
//  @VisibleForTesting
//  RssTezFetcherTask constructRssFetcherForPartition(InputHost inputHost, Configuration conf) {
//    RssTezFetcherTask fetcher=  new RssTezFetcherTask(inputContext, conf, inputHost, null);
//    return fetcher;
//
//
//
...
//  }
  ...
  public void addKnownInput(String hostName, int port,
    ...
    LOG.info("AddKnowInput, hostname:{}, port:{}, srcAttemtId:{}, srcPhysicalIndex:{}",
        hostName, port, srcAttemptIdentifier, srcPhysicalIndex);
    RssTezUtils.parseRssWorker(rssPartitionWorkers, inputContext.getTaskVertexName(), hostName);
    ...
    try {
      ...
      int numPendingHosts = numAddedPendingHosts.incrementAndGet();
      LOG.info("AddKnownInput, host:{}", host);
  ...
  public void addCompletedInputWithNoData(
    ...
    LOG.info("NumNoDataInput:{}.", numNoDataInput.get());
  }
...
//      if (!completedInputSet.get(inputIdentifier)) {
  ...
  public boolean isAllPartitionFetched() {
    ...
    try {
      if (!allRssPartition.containsAll(successRssPartitionSet)) {
        LOG.error("Failed to check partition, all partition:{}, success partiton:{}", allRssPartition, successRssPartitionSet);
      }
      LOG.info("IsAllInputsFetched, all:{}, success:{}, r:{}", allRssPartition, successRssPartitionSet,
          allRssPartition.containsAll(successRssPartitionSet));
      return successRssPartitionSet.size() >= allRssPartition.size();
  ...
  public FetchedInput getNextInput() throws InterruptedException {
    ...
    if (completedInputs.peek() == null) {
        LOG.info("GetNextInput, fetchedInputttt, null");
        while (true) {
  ...
  private class FetchFutureCallback implements FutureCallback<FetchResult> {
    ...
    public FetchFutureCallback(RssTezFetcherTask fetcher) {
    ...
    public void onSuccess(FetchResult result) {
      ...
      if (isShutdown.get()) {
        ...
        lock.lock();  // 加锁，确保successRssPartitionSet和runningRssPartitionMap的操作是原子的。
        try {
    ...
    public void onFailure(Throwable t) {
      ...
      if (isShutdown.get()) {
        ...
        runningRssPartitionMap.remove(fetcher.getPartitionId());
        pendingPartition.add(fetcher.getPartitionId());
