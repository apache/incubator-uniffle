...
@Unstable
@Private
public class RssUnorderedKVReader<K, V> extends KeyValueReader {
  ...
  public RssUnorderedKVReader(ShuffleManager shuffleManager, Configuration conf,
  ...
  public float getProgress() throws IOException, InterruptedException {
    ...
    if (totalFileBytes.get() > 0 && numInputs > 0) {
      return ((1.0f) * (totalBytesRead.get() + ((currentReader != null) ? currentReader.bytesRead : 0.0f))
  ...
  public IFile.Reader openIFileReader(FetchedInput fetchedInput)
    ...
    if (fetchedInput.getType() == Type.MEMORY) {
      ...
      IFile.Reader reader =  new IFile.Reader(fetchedInput.getInputStream(),
          fetchedInput.getSize(), codec, null, null, ifileReadAhead,
          ifileReadAheadLength, ifileBufferSize);
      reader.disableChecksumValidation();
      return reader;
    }
  }
}
