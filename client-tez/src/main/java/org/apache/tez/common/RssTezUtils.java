...
public class RssTezUtils {
  ...
  public static long getInitialMemoryRequirement(Configuration conf, long maxAvailableTaskMemory) {
    long initialMemRequestMb = conf.getLong(RssTezConfig.RSS_RUNTIME_IO_SORT_MB, RssTezConfig.RSS_DEFAULT_RUNTIME_IO_SORT_MB);
    ...
    Preconditions.checkArgument(initialMemRequestMb > 0 && reqBytes < maxAvailableTaskMemory,
            RssTezConfig.RSS_RUNTIME_IO_SORT_MB + initialMemRequestMb
                    + " should be " + "larger than 0 and should be less than the available task memory (MB):" + (maxAvailableTaskMemory >> 20));
  ...
  public static String uniformPartitionHostInfo(Map<Integer, List<ShuffleServerInfo>> map) {
    ...
    for (Integer pid : pidSet) {
      for (ShuffleServerInfo shuffleServerInfo : map.get(pid)) {
        tmp = pid + UNDERLINE_DELIMITER + shuffleServerInfo.getHost() + COLON_DELIMITER + shuffleServerInfo.getPort();
        res.add(tmp);
      }
    }
  ...
  public static Roaring64NavigableMap fetchAllRssTaskIds(Set<InputAttemptIdentifier> successMapTaskAttempts,
    ...
    for (InputAttemptIdentifier inputAttemptIdentifier: successMapTaskAttempts) {
      ...
      if (!mapIndexBitmap.contains(taskId)) {
        ...
        LOG.info("FetchAllRssTaskIds, successMapTaskAttempts:{}, totalMapsCount:{}, appAttemptId:{}, mapIndex:{}, totalMapsCount:{}, taskId: {} ",
  ...
  public static void parseRssWorker(Map<Integer, Set<ShuffleServerInfo>> rssWorker, String vertexName, String hostnameInfo) {
