...
public class PartialInputStream extends InputStream {
  ...
  public PartialInputStream(SeekableByteChannel ch, long start, long end, Closeable closeable)
      throws IOException {
    if (start < 0) {
  ...
  private static PartialInputStream newInputStream(SeekableByteChannel ch, long start, long end, Closeable closeable)
      throws IOException {
    if (ch == null)
      throw new NullPointerException("channel is null!");
    return new PartialInputStream(ch, start, end, closeable);
  }
  ...
  public static PartialInputStream newInputStream(File file, long start, long end)
    ...
    long size = fc.size();
    return newInputStream(input.getChannel(), start, Math.min(end, size), () -> {
      input.close();
    });
  }

  public static PartialInputStream newInputStream(byte[] bytes, long start, long end) throws IOException {
