...
public class CoalescedCollectorRegistry extends CollectorRegistry {
  private List<CollectorRegistry> collectorRegistryList;

  public CoalescedCollectorRegistry() {
    collectorRegistryList = new ArrayList<>();
  }

  @Override
  public Enumeration<Collector.MetricFamilySamples> metricFamilySamples() {
    return new CoalescedEnumeration((index) -> collectorRegistryList.get(index).metricFamilySamples());
  }

  @Override
  public Enumeration<Collector.MetricFamilySamples> filteredMetricFamilySamples(Set<String> includedNames) {
    return new CoalescedEnumeration((index) -> collectorRegistryList.get(index)
  ...
  public Double getSampleValue(String name, String[] labelNames, String[] labelValues) {
    Double ret = null;
    for (CollectorRegistry collectorRegistry : collectorRegistryList) {
  ...
  public void addCollectorRegistry(CollectorRegistry collectorRegistry) {
    collectorRegistryList.add(collectorRegistry);
  }
  ...
  private class CoalescedEnumeration implements Enumeration<Collector.MetricFamilySamples> {
    ...
    public boolean hasMoreElements() {
      if (currentEnumeration == null || !currentEnumeration.hasMoreElements()) {
        if (index >= collectorRegistryList.size()) {
          return false;
        }
