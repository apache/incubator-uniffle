...
public class PartialInputStreamImpl extends PartialInputStream {
  ...
  private static PartialInputStreamImpl newInputStream(SeekableByteChannel ch, long start, long end, Closeable closeable)
      throws IOException {
    if (ch == null)
  ...
  public static PartialInputStreamImpl newInputStream(File file, long start, long end)
    ...
    long size = fc.size();
    return newInputStream(fc, start, Math.min(end, size), () -> {
      input.close();
    });
  }

  public static PartialInputStreamImpl newInputStream(byte[] bytes, long start, long end) throws IOException {
