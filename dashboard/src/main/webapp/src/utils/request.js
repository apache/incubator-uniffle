...
const axiosInstance = {
    getFontEndAxiosInstance(){
        return fontEndAxiosInstance
    },
...
fontEndAxiosInstance.interceptors.request.use(config => {
...
fontEndAxiosInstance.interceptors.response.use(response => {
    return response;
}, error => {
    return error;
})
